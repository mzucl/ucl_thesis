...
for i = 1:k
    ...
    for s = 1:stabilityRuns
        ...
        model = BGFA({X_tr', y_tr'}, 1, K, 'B', 10000, 1e-5); % Views are expected in DxN
    ...
    % Threshold on train data
    MU_Z = sigma_Z * (W1' * T1 * (X_tr' - mu1));
    
    pred_tr = Bound.sigma(W2 * MU_Z + mu2);
    [fpr, tpr, thresholds, auc] = perfcurve(y_tr', pred_tr, 1);

    % Calculate G-means
    gmeans = sqrt(tpr .* (1 - fpr));
    
    % Find the index of the maximum G-means value
    [~, idx] = max(gmeans);
    ...
    pred_te = Bound.sigma(W2 * MU_Z + mu2);
